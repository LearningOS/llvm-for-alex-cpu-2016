//===-- V9ISelLowering.cpp - V9 DAG Lowering Implementation ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the interfaces that V9 uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
static bool CC_V9_Assign_SRet(unsigned &ValNo, MVT &ValVT,
...
static bool CC_V9_Assign_Split_64(unsigned &ValNo, MVT &ValVT,
                                     MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                     ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
...
static bool CC_V9_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
                                         MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                         ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
  static const MCPhysReg RegList[] = {
...
static bool CC_V964_Full(unsigned &ValNo, MVT &ValVT,
                            MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool CC_V964_Half(unsigned &ValNo, MVT &ValVT,
                            MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
#include "V9GenCallingConv.inc"

// The calling conventions in V9CallingConv.td are described in terms of the
...
SDValue
V9TargetLowering::LowerReturn(SDValue Chain,
...
SDValue
V9TargetLowering::LowerReturn_32(SDValue Chain,
                                    CallingConv::ID CallConv, bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    SDLoc DL, SelectionDAG &DAG) const {
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_V932);
  ...
  if (MF.getFunction()->hasStructRetAttr()) {
    V9MachineFunctionInfo *SFI = MF.getInfo<V9MachineFunctionInfo>();
...
SDValue
V9TargetLowering::LowerReturn_64(SDValue Chain,
                                    CallingConv::ID CallConv, bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    SDLoc DL, SelectionDAG &DAG) const {
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_V964);
...
LowerFormalArguments_32(SDValue Chain,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_V932);
...
LowerFormalArguments_64(SDValue Chain,
  ...
  // Analyze arguments according to CC_V964.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_V964);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    assert(VA.isMemLoc());
    // The CC_V964_Full/Half functions compute stack offsets relative to the
...
SDValue
V9TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                               SmallVectorImpl<SDValue> &InVals) const {
  if (Subtarget->is64Bit())
...
SDValue
V9TargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  ...
  // V9 target does not yet support tail call optimization.
  isTailCall = false;
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_V932);
  ...
  const V9RegisterInfo *TRI = Subtarget->getRegisterInfo();
  const uint32_t *Mask =
      ((hasReturnsTwice)
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_V932);
...
unsigned
V9TargetLowering::getSRetArgSize(SelectionDAG &DAG, SDValue Callee) const
{
  const Function *CalleeFn = nullptr;
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    CalleeFn = dyn_cast<Function>(G->getGlobal());
...
SDValue
V9TargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  SelectionDAG &DAG = CLI.DAG;
  SDLoc DL = CLI.DL;
  SDValue Chain = CLI.Chain;
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_V964);

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_V964 includes all arguments.
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_V964);
...
TargetLowering::AtomicExpansionKind V9TargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
V9TargetLowering::V9TargetLowering(TargetMachine &TM,
                                         const V9Subtarget &STI)
  ...
  // V9 doesn't have i1 sign extending load
  ...
  // V9 doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8 , Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1 , Expand);

  // V9 has no REM or DIVREM operations.
  ...
  // V9 has no select or setcc: expand to SELECT_CC.
  ...
  // V9 doesn't have BRCOND either, it has BR_CC.
  ...
  if (!Subtarget->isV9()) {
    // V9V8 does not have FNEGD and FABSD.
  ...
  // FIXME: V9 provides these multiplies, but we don't have them yet.
...
const char *V9TargetLowering::getTargetNodeName(unsigned Opcode) const {
...
EVT V9TargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                            EVT VT) const {
  if (!VT.isVector())
    return MVT::i32;
...
/// combiner.
void V9TargetLowering::computeKnownBitsForTargetNode
                                (const SDValue Op,
                                 APInt &KnownZero,
                                 APInt &KnownOne,
...
SDValue V9TargetLowering::withTargetFlags(SDValue Op, unsigned TF,
...
SDValue V9TargetLowering::makeHiLoPair(SDValue Op,
...
SDValue V9TargetLowering::makeAddress(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getRelocationModel() == Reloc::PIC_) {
    ...
    SDValue HiLo = makeHiLoPair(Op, V9MCExpr::VK_V9_GOT22,
                                V9MCExpr::VK_V9_GOT10, DAG);
  ...
  case CodeModel::Small:
    // abs32.
    return makeHiLoPair(Op, V9MCExpr::VK_V9_HI,
                        V9MCExpr::VK_V9_LO, DAG);
  case CodeModel::Medium: {
    // abs44.
    SDValue H44 = makeHiLoPair(Op, V9MCExpr::VK_V9_H44,
                               V9MCExpr::VK_V9_M44, DAG);
    H44 = DAG.getNode(ISD::SHL, DL, VT, H44, DAG.getConstant(12, DL, MVT::i32));
    SDValue L44 = withTargetFlags(Op, V9MCExpr::VK_V9_L44, DAG);
    L44 = DAG.getNode(SPISD::Lo, DL, VT, L44);
    return DAG.getNode(ISD::ADD, DL, VT, H44, L44);
  }
  case CodeModel::Large: {
    // abs64.
    SDValue Hi = makeHiLoPair(Op, V9MCExpr::VK_V9_HH,
                              V9MCExpr::VK_V9_HM, DAG);
    Hi = DAG.getNode(ISD::SHL, DL, VT, Hi, DAG.getConstant(32, DL, MVT::i32));
    SDValue Lo = makeHiLoPair(Op, V9MCExpr::VK_V9_HI,
                              V9MCExpr::VK_V9_LO, DAG);
    return DAG.getNode(ISD::ADD, DL, VT, Hi, Lo);
  }
  }
...
SDValue V9TargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue V9TargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue V9TargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue V9TargetLowering::LowerGlobalTLSAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    unsigned HiTF = ((model == TLSModel::GeneralDynamic)
                     ? V9MCExpr::VK_V9_TLS_GD_HI22
                     : V9MCExpr::VK_V9_TLS_LDM_HI22);
    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
                     ? V9MCExpr::VK_V9_TLS_GD_LO10
                     : V9MCExpr::VK_V9_TLS_LDM_LO10);
    unsigned addTF = ((model == TLSModel::GeneralDynamic)
                      ? V9MCExpr::VK_V9_TLS_GD_ADD
                      : V9MCExpr::VK_V9_TLS_LDM_ADD);
    unsigned callTF = ((model == TLSModel::GeneralDynamic)
                       ? V9MCExpr::VK_V9_TLS_GD_CALL
                       : V9MCExpr::VK_V9_TLS_LDM_CALL);
    ...
    SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                 withTargetFlags(Op, V9MCExpr::VK_V9_TLS_LDO_HIX22, DAG));
    SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                 withTargetFlags(Op, V9MCExpr::VK_V9_TLS_LDO_LOX10, DAG));
    HiLo =  DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
                   withTargetFlags(Op, V9MCExpr::VK_V9_TLS_LDO_ADD, DAG));
  }

  if (model == TLSModel::InitialExec) {
    unsigned ldTF     = ((PtrVT == MVT::i64)? V9MCExpr::VK_V9_TLS_IE_LDX
                         : V9MCExpr::VK_V9_TLS_IE_LD);
    ...
    SDValue TGA = makeHiLoPair(Op,
                               V9MCExpr::VK_V9_TLS_IE_HI22,
                               V9MCExpr::VK_V9_TLS_IE_LO10, DAG);
    ...
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT,
                       ...
                       withTargetFlags(Op,
                                       V9MCExpr::VK_V9_TLS_IE_ADD, DAG));
  }

  assert(model == TLSModel::LocalExec);
  SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                  withTargetFlags(Op, V9MCExpr::VK_V9_TLS_LE_HIX22, DAG));
  SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                  withTargetFlags(Op, V9MCExpr::VK_V9_TLS_LE_LOX10, DAG));
...
SDValue
V9TargetLowering::LowerF128_LibCallArg(SDValue Chain, ArgListTy &Args,
...
SDValue
V9TargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
...
SDValue
V9TargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
                                      unsigned &SPCC,
                                      SDLoc DL,
                                      SelectionDAG &DAG) const {
...
MachineBasicBlock *
V9TargetLowering::EmitInstrWithCustomInserter(MachineInstr *MI,
...
MachineBasicBlock*
V9TargetLowering::expandSelectCC(MachineInstr *MI,
...
//===----------------------------------------------------------------------===//
//                         V9 Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
V9TargetLowering::ConstraintType
V9TargetLowering::getConstraintType(StringRef Constraint) const {
...
TargetLowering::ConstraintWeight V9TargetLowering::
...
V9TargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
...
bool
V9TargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // The V9 target isn't yet aware of offsets.
  return false;
}

void V9TargetLowering::ReplaceNodeResults(SDNode *N,
