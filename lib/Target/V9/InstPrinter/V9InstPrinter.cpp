//===-- V9InstPrinter.cpp - Convert V9 MCInst to assembly syntax -----==//
//
...
//===----------------------------------------------------------------------===//
//
// This class prints an V9 MCInst to a .s file.
//
//===----------------------------------------------------------------------===//

#include "V9InstPrinter.h"
#include "V9.h"
...
// The generated AsmMatcher V9GenAsmWriter uses "V9" as the target
...
#include "V9GenAsmWriter.inc"

bool V9InstPrinter::isV9(const MCSubtargetInfo &STI) const {
  return (STI.getFeatureBits()[V9::FeatureV9]) != 0;
}

void V9InstPrinter::printRegName(raw_ostream &OS, unsigned RegNo) const
{
  OS << '%' << StringRef(getRegisterName(RegNo)).lower();
}

void V9InstPrinter::printInst(const MCInst *MI, raw_ostream &O,
                                 StringRef Annot, const MCSubtargetInfo &STI) {
  if (!printAliasInstr(MI, STI, O) && !printV9AliasInstr(MI, STI, O))
    printInstruction(MI, STI, O);
  printAnnotation(O, Annot);
}

bool V9InstPrinter::printV9AliasInstr(const MCInst *MI,
...
void V9InstPrinter::printOperand(const MCInst *MI, int opNum,
                                    const MCSubtargetInfo &STI,
                                    raw_ostream &O) {
...
void V9InstPrinter::printMemOperand(const MCInst *MI, int opNum,
                                       const MCSubtargetInfo &STI,
...
void V9InstPrinter::printCCOperand(const MCInst *MI, int opNum,
                                      const MCSubtargetInfo &STI,
                                      raw_ostream &O) {
...
bool V9InstPrinter::printGetPCX(const MCInst *MI, unsigned opNum,
                                   const MCSubtargetInfo &STI,
                                   raw_ostream &O) {
  llvm_unreachable("FIXME: Implement V9InstPrinter::printGetPCX.");
  return true;
}
