//===-- V9InstrInfo.cpp - V9 Instruction Information ----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the V9 implementation of the TargetInstrInfo class.
//
//===----------------------------------------------------------------------===//
...
void V9InstrInfo::anchor() {}

V9InstrInfo::V9InstrInfo(V9Subtarget &ST)
    : V9GenInstrInfo(SP::ADJCALLSTACKDOWN, SP::ADJCALLSTACKUP), RI(),
      Subtarget(ST) {}
...
bool V9InstrInfo::AnalyzeBranch(MachineBasicBlock &MBB,
...
unsigned
V9InstrInfo::InsertBranch(MachineBasicBlock &MBB,MachineBasicBlock *TBB,
                             MachineBasicBlock *FBB,
                             ArrayRef<MachineOperand> Cond,
                             DebugLoc DL) const {
  assert(TBB && "InsertBranch must not be told to insert a fallthrough");
  assert((Cond.size() == 1 || Cond.size() == 0) &&
         "V9 branch conditions should have one component!");
...
unsigned V9InstrInfo::RemoveBranch(MachineBasicBlock &MBB) const
{
...
bool V9InstrInfo::ReverseBranchCondition(
...
void V9InstrInfo::copyPhysReg(MachineBasicBlock &MBB,
...
unsigned V9InstrInfo::getGlobalBaseReg(MachineFunction *MF) const
{
  V9MachineFunctionInfo *V9FI = MF->getInfo<V9MachineFunctionInfo>();
  unsigned GlobalBaseReg = V9FI->getGlobalBaseReg();
  if (GlobalBaseReg != 0)
  ...
  V9FI->setGlobalBaseReg(GlobalBaseReg);
  return GlobalBaseReg;
}
