...
namespace llvm {
  ...
  FunctionPass *createV9ISelDag(V9TargetMachine &TM);
  FunctionPass *createV9DelaySlotFillerPass(TargetMachine &TM);

  void LowerV9MachineInstrToMCInst(const MachineInstr *MI,
                                      MCInst &OutMI,
                                      AsmPrinter &AP);
} // end namespace llvm;

namespace llvm {
  // Enums corresponding to V9 condition codes, both icc's and fcc's.  These
