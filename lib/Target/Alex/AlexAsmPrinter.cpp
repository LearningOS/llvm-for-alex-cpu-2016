...
bool AlexAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNum,
...
bool AlexAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
    ...
    O << Offset << "($" << AlexInstPrinter::getRegisterName(MO.getReg()) << ")";

    return false;
}

void AlexAsmPrinter::printOperand(const MachineInstr *MI, int opNum,
                                  raw_ostream &O) {
    const MachineOperand &MO = MI->getOperand(opNum);
    bool closeP = false;
    ...
    case MachineOperand::MO_Register:
        O << '$'
        << StringRef(AlexInstPrinter::getRegisterName(MO.getReg())).lower();
        break;
