...
V9CpuAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOpnd(I) {}

// Add I to the instruction sequences.
void V9CpuAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void V9CpuAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
    GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
    AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}

void V9CpuAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
//    GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
//    AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void V9CpuAnalyzeImmediate::GetInstSeqLsSHL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void V9CpuAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void V9CpuAnalyzeImmediate::ReplaceADDiuSHLWithLUi(InstSeq &Seq) {
...
void V9CpuAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const V9CpuAnalyzeImmediate::InstSeq
&V9CpuAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
